...
module.exports = {
  ...
  getById: id => albumes.find(a => a.id == id),

  getByArtist: artistaId =>
    albumes.filter(a => a.artistaId == artistaId),

  add: album => albumes.push(album),

  update: album => {
    const index = albumes.findIndex(a => a.id == album.id);
    if (index !== -1) albumes[index] = album;
  },

  delete: id => {
    const nuevaLista = albumes.filter(a => a.id != id);
    albumes = nuevaLista;
  }
};
